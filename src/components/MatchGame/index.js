...
class MatchGame extends Component {
  state = {
    ...
    timer: 60,
    matchImage: imagesList[randomNumber].imageUrl,
    altImage: 'match',
  }
  ...
  clickThumbnailItem = id => {
    ...
    const clickedImagesListLength = clickedImagesList.length
    const isClickingSameImage = id === imagesList[randomNumber].id

    if (timer > 0) {
      if (isClickingSameImage) {
        randomNumber = Math.floor(Math.random() * imagesList.length - 1)
        this.setState(prevState => ({
          clickedImagesList: [...clickedImagesList, id],
          matchImage: imagesList[randomNumber].imageUrl,
          altImage: imagesList[randomNumber].category,
        }))
      } else {
        this.clearTimerInterval()
        this.setState({isGamePlaying: false, timer: 0})
      }
    } else {
      this.clearTimerInterval()
      this.setState({isGamePlaying: false, timer: 0})
  ...
  getActiveImagesList = () => {
    ...
    const getResults = imagesList.filter(
      eachImageInfo => eachImageInfo.category === activeTabId,
    )

    return getResults
  }

  decreasingTimerValue = () => {
  ...
  renderMatchGameItemsView = () => {
    ...
    const thumbnailsList = this.getActiveImagesList()

    return (
      <div className="tab-item-and-thumbnail-containers">
        <img src={matchImage} alt={altImage} className="match-img" />
        ...
        <ul className="images-list-container">
          {thumbnailsList.map(eachThumbnail => (
            <ThumbnailItem
  ...
  renderScoreCardView = () => {
    ...
    const userScore = clickedImagesList.length

    return <ScoreCard score={userScore} resetGame={this.resetGame} />
  }

  componentDidMount = () => {
    this.intervalId = setInterval(this.decreasingTimerValue, 1000)
  }
  ...
  render() {
    ...
    return (
      <div className="match-game-app-bg-container">
        <div className="nav-bar-body-container">
            ...
            <div className="navbar-card">
              ...
              <ul>
                <NavBar
                  ...
                  getTimeElapsedSeconds={this.getTimeElapsedSeconds}
                  timerValue={timer}
                />
              </ul>
            </div>
